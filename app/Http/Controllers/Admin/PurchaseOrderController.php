...
class PurchaseOrderController extends GlobalController
{
    ...
    public function viewPurchaseOrder(ZohoBookService $zohoBook, $id)
    {
        ...
        $html = $zohoBook->getPurchaseOrderHtml(base64_decode($id));
        return view('admin.po.view_detail', compact('html'));
    }

    public function purchaseorderdownload(ZohoBookService $zohoBook, $id)
    {
        try { // adjust service class name if different
            $purchaseorderId = base64_decode($id);
            $pdfContent = $zohoBook->getpurchaseorderPdf($purchaseorderId);

            $fileName = "purchaseorder_{$purchaseorderId}.pdf";
