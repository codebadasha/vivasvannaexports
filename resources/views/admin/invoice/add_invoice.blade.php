                    ...
                    <div class="card-body">
                        ...
                        <form class="custom-validation" action="{{ route('admin.po.saveInvoice') }}" method="post" id="addClientCompany" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3">
                                    <label>Select Purchase Order <span class="mandatory">*</span></label>
                                    <select class="form-control" name="po_id" required id="poId">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="po_number" class="form-control" placeholder="PO Number" value="" id="poNumber" readonly required>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="due_days" class="form-control" placeholder="Due Days" value="" id="dueDays" required>
                                </div>

                                <div class="mb-3">
                                    <label>Due Date <span class="mandatory">*</span></label>
                                    <input type="date" name="due_date" class="form-control" placeholder="Due Days" value="" id="due_date" required>
                                </div>
...
<script type="text/javascript">
    $(document).on('change','#client',function(){
        $.ajax({
            type : 'post',
            url : '/admin/invoice/get-all-purchase-order',
            data : { 
                id : $(this).val()
            },
            success:function(data){
                $('#poId').empty();
                $('#poId').append($('<option>', {value: "",text: "Select Purchase Order"}));
                $.each(data, function(index, option) {
                    var newOption = $('<option>', {
                        value: option.id,
                        text: option.po_number
                    });
                    $('#poId').append(newOption);
                });
            }
        })
    })
    ...
    $(document).on('change','#poId',function(){
        $.ajax({
            type : 'post',
            url : '/admin/invoice/get-all-po-items',
            data : { 
                id : $(this).val()
            },
            success:function(data){
                $('#productId').empty();
                $('#productId').append($('<option>', {value: "",text: "Select Purchase Order"}));
                $('#poNumber').val(data.po_number)
                $('#dueDays').val(data.credit_day)
                generateData(data.credit_day)
                $.each(data.item, function(index, option) {
                    var newOption = $('<option>', {
                        value: option.id,
                        text: option.varation.product.product_type+' - '+option.varation.grade
                    });
                    $('#productId').append(newOption);
                });
            }
        })
    })
