                        ...
                        <div class="card">
                            <div class="card-body row">
                                ...
                                <div class="form-group col-md-4 mb-3">
                                    ...
                                    <select class="form-control select2" name="investor[]" data-placeholder="Select Invstors" multiple>
                                        @forelse(\App\Models\Investor::where('is_active',1)->where('is_delete',0)->get() as $ik => $iv)
                                        <option value="{{ $iv->id }}" {{ in_array($iv->id, old('investor', $investor)) ? 'selected' : '' }}>{{ $iv->name }}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
